...
var import_supabase_js, createAdminClient;
...
var qr_exports = {};
__export(qr_exports, {
  generateAndUploadQRCode: () => generateAndUploadQRCode
});
async function generateAndUploadQRCode(clientId, targetUrl) {
  const supabase = createAdminClient();
  try {
    const qrBuffer = await import_qrcode.default.toBuffer(targetUrl, {
    ...
    const { data: uploadData, error: uploadError } = await supabase.storage.from("qrcodes").upload(filepath, qrBuffer, {
    ...
    const { data: { publicUrl } } = supabase.storage.from("qrcodes").getPublicUrl(filepath);
    return {
      success: true,
      imagePath: filepath,
      imageUrl: publicUrl
...
async function approveProvider(providerId, stateCode, providerType) {
  const supabase = createAdminClient();
  try {
    const year = (/* @__PURE__ */ new Date()).getFullYear();
    const typeCode = providerType === "Hospital" ? "HOS" : providerType === "Clinic" ? "CLI" : providerType === "Individual Doctor" ? "DOC" : providerType === "Pharmacy" ? "PHY" : providerType === "Diagnostic Center" ? "DIA" : "OTH";
    ...
    const { generateAndUploadQRCode: generateAndUploadQRCode2 } = await Promise.resolve().then(() => (init_qr(), qr_exports));
    const { generateToken: generateToken2 } = await Promise.resolve().then(() => (init_crypto(), crypto_exports));
    const formsBaseUrl = process.env.NEXT_PUBLIC_FORMS_URL || "https://healthmetro-forms.vercel.app";
    const token = generateToken2(clientId);
    const registrationUrl = `${formsBaseUrl}/register/customer?client_id=${clientId}&token=${token}&src=qr`;
    const qrResult = await generateAndUploadQRCode2(clientId, registrationUrl);
    ...
    const { error: qrDbError } = await supabase.from("qr_codes").upsert({
      client_id: clientId,
    ...
    (0, import_cache.revalidatePath)("/providers");
